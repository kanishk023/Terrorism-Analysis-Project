...
# express is a framework or library in plotly which helps us in creating figures
from dash.exceptions import PreventUpdate


#app is a global variable
#app is a class  in dash library-python supports oops
app = dash.Dash()
...
#with Dash we can create highly custom user interfaces in pure python

# Layout of your page
def create_app_ui():
  # Create the UI of the Webpage here
    #we can give title of the page using title parameter
  ...
  dcc.Tabs(id="Tabs", value="Map",children=[
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map", children=[
          dcc.Tabs(id = "subtabs", value = "WorldMap",children = [
              ...
              ]),#according to the client requirement we need two tabs
          #1.map tool,2.chart tool
          dcc.Dropdown(
              id='month', 
                options=month_list,#options-to show in dropdown
                placeholder='Select Month',
                multi = True#(for multiple inputs)
                  ),
          dcc.Dropdown(
                id='date', 
          ...
          html.Br()
    ]),#dropdowns we get for world map tool
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart", children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
            ...
            html.Hr(),
            html.Br(),
            dcc.RangeSlider(#if we want data between specific years
                    id='cyear_slider',
                    min=min(year_list),
                    max=max(year_list),
                    value=[min(year_list),max(year_list)],
  ...
  return main_layout
#range sliders-if we want data between specific years
  # we can slide on both sides

# Callbacks-calling a function without using the function name 
...
#A Mapbox Style object as defined at https://docs.mapbox.com/mapbox-gl-js/        
        mapFigure = px.scatter_mapbox(new_df,
          lat="latitude", 
          lon="longitude",
          color="attacktype1_txt",#different colors for attack types
          hover_name="city", 
...
def main():
  ...
  #https://www.favicon.cc/ -download the ico file and store in assets directory
  #icon for the application
  #we no need to write any code for this
